...
def hand_rank(cards):
    ...
    counts = {rank: list(ranks).count(rank) for rank in set(ranks)}
    count_list = sorted(counts.items(), key=lambda x: (-x[1], -x[0]))
    count_values = tuple([c[1] for c in count_list])
    ordered_ranks = [r for r, _ in count_list for _ in range(counts[r])]
...
def best_five(cards):
    # Sort the cards by rank, keeping the suit
    sorted_cards = sorted(cards, key=lambda x: (x[0], x[1]), reverse=True)

    # Check for flush
    suits = [card[1] for card in sorted_cards]
    flush_suits = [suit for suit in set(suits) if suits.count(suit) >= 5]
    
    best_hand = (0, [])
    
    # If we have a flush, evaluate the top 5 flush cards
    if flush_suits:
        for flush_suit in flush_suits:
            flush_cards = [card for card in sorted_cards if card[1] == flush_suit]
            flush_hand = hand_rank(flush_cards[:5])  # Only the top 5 cards of the flush
            if flush_hand > best_hand:
                best_hand = flush_hand

    # Check for straight (non-flush)
    unique_ranks = sorted(set([card[0] for card in sorted_cards]), reverse=True)
    for i in range(len(unique_ranks) - 4):
        if unique_ranks[i] - unique_ranks[i+4] == 4:  # Consecutive sequence
            straight_hand = hand_rank(sorted_cards[:5])  # Top 5 cards
            if straight_hand > best_hand:
                best_hand = straight_hand

    # Return the best hand found
    return best_hand


def estimate_strength(my_cards, board_cards, iterations=150):
...
class Player(Bot):
    ...
    def get_action(self, game_state, round_state, active):
        ...
        # Postflop logic
        strength = estimate_strength(my_cards, board_cards, iterations=150)
