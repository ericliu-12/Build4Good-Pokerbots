...
SUITS = 'cdhs'
DECK = [r + s for r in RANKS for s in SUITS]
CARD_CACHE = {c: (RANKS.index(c[0]), SUITS.index(c[1])) for c in DECK}
...
def fast_eval(my_cards, board_cards):
    parsed_my = [parse_card(c)[0] for c in my_cards]
    parsed_board = [parse_card(c)[0] for c in board_cards]
    all_ranks = parsed_my + parsed_board
...
class Player(Bot):
    ...
    def get_action(self, game_state, round_state, active):
        ...
        strength = fast_eval(my_cards, board_cards)
        pot_odds = continue_cost / (pot_total + 1e-9)
        bluff_chance = 0.15 if self.rounds_played > 5 and self.opp_aggression / self.rounds_played > 0.5 else 0.05
